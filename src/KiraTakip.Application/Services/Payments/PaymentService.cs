    ...
    IPaymentBusinessRules paymentBusinessRules) : IPaymentService, ITransactionalService
{
    ...
    public async Task ApproveAsync(ApprovePaymentInput input)
    {
        ...
        var payment = Guard.NotFound(
            ...
            "PAYMENT_NOT_FOUND");
        Guard.Conflict(
            !PaymentStatusTransitionPolicy.CanTransition(payment.Status, PaymentStatus.Approved),
    ...
    public async Task RejectAsync(RejectPaymentInput input)
    {
        ...
        var payment = Guard.NotFound(
            ...
            "PAYMENT_NOT_FOUND");
        Guard.Conflict(
            !PaymentStatusTransitionPolicy.CanTransition(payment.Status, PaymentStatus.Rejected),
    ...
    private async Task<List<ChargeLineItemPaymentBalanceDto>> GetPayableLineItemBalancesAsync(int chargeId)
    {
        ...
        return balances.Where(balance => PaymentAmountPolicy.HasAvailableBalance(balance.AvailableAmount)).ToList();
    }
