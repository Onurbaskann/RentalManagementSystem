...
namespace KiraTakip.Services.Tenants;
    ...
    IPaymentAllocationRepository paymentAllocationRepository) : ITenantPanelService
{
    public async Task<TenantPanelDashboardDto> GetDashboardAsync(GetTenantPanelDashboardInput input)
    {
        ...
        {
            var dayDifference = TenantPanelSchedulePolicy.CalculateDueDayDifference(charge.DueDate, input.Today);
            var urgency = TenantPanelSchedulePolicy.DetermineUpcomingChargeUrgency(dayDifference);
            var borderColor = urgency switch
            {
                UpcomingChargeUrgency.Overdue => "red",
                UpcomingChargeUrgency.DueSoon => "amber",
                _ => "emerald"
            };
