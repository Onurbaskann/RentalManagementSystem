    ...
    TimeProvider timeProvider) : IReservationBusinessRules
{
        ...
        bool validatePastDate = true)
    {
        ...
        Guard.InvalidField(
            !ReservationSchedulePolicy.HasValidDateRange(startDate, endDate),
        ...
        Guard.InvalidField(
            validatePastDate
                && !ReservationSchedulePolicy.MeetsMinimumAdvance(
                    startDate,
                    currentTime,
        ...
        Guard.InvalidField(
            !ReservationSchedulePolicy.MeetsMinimumDuration(
                startDate,
                endDate,
        ...
        Guard.InvalidField(
            !ReservationSchedulePolicy.DoesNotExceedMaximumDuration(
                startDate,
                endDate,
        ...
        Guard.InvalidField(
            !ReservationSchedulePolicy.IsWithinMaximumAdvance(
                startDate,
                currentTime,
    ...
    public void EnsureContentIsValid(ReservationContentPolicyInput input)
    {
        ...
        Guard.InvalidField(
            !ReservationAttendeePolicy.IsWithinMaximumCount(
                input.Attendees.Count,
                policy.MaximumAttendeeCount),
            nameof(input.Attendees),
            $"En fazla {policy.MaximumAttendeeCount} katılımcı eklenebilir.",
            "RESERVATION_ATTENDEE_LIMIT_EXCEEDED");
        Guard.InvalidField(
            !ReservationAttendeePolicy.HasExactlyOneOwner(
                input.Attendees.Count(attendee => attendee.IsReservationOwner)),
            nameof(input.Attendees),
    ...
    {
        var invalid = !ReservationUnitPolicy.IsReservable(
            unit.IsUnitActive,
        ...
        bool canOverrideTimeRestriction)
    {
        ...
        Guard.Conflict(
            !ReservationLifecycle.CanCancel(reservation.Status),
            "Bu durumdaki rezervasyon iptal edilemez.",
            "RESERVATION_CANNOT_BE_CANCELLED");
        Guard.Conflict(
            !canOverrideTimeRestriction
            && ReservationSchedulePolicy.HasReachedModificationCutoff(
                reservation.StartDate,
                GetCurrentTime(),
                policy.ModificationCutoffMinutes),
        ...
        string? overrideReason)
    {
        var policy = policyProvider.Current;
        Guard.Conflict(
            !ReservationLifecycle.CanModify(reservation.Status),
            "Bu durumdaki rezervasyon güncellenemez.",
            "RESERVATION_CANNOT_BE_MODIFIED");

        var cutoffExceeded = ReservationSchedulePolicy.HasReachedModificationCutoff(
            reservation.StartDate,
            GetCurrentTime(),
            policy.ModificationCutoffMinutes);
        Guard.Conflict(
        ...
        Guard.Against(
            cutoffExceeded
                && canOverrideTimeRestriction
                && !ReservationSchedulePolicy.HasValidOverrideReason(overrideReason, 450),
