...
namespace KiraTakip.Services.Notifications;

public class SmtpMailService(IOptions<SmtpSettings> options, ILogger<SmtpMailService> logger) : IMailService
{
    ...
    public async Task SendAsync(string toAddress, string toName, string subject, string htmlBody, CancellationToken ct = default)
    {
        ...
        logger.LogInformation("Mail gönderildi: {To} — {Subject}", toAddress, subject);
    }
}
