...
public class PropertyRepository : BaseRepository<Property>, IPropertyRepository
{
    ...
    public async Task<List<BirimLookupDto>> GetBosBirimlerAsync(List<int>? yetkiliPropertyIds)
    {
        var now = DateTime.Now;
    ...
    public async Task<List<BirimLookupDto>> GetTumBirimlerAsync(List<int>? yetkiliPropertyIds)
    {
        var query = _ctx.Units.AsNoTracking().AsQueryable();
        if (yetkiliPropertyIds != null)
            query = query.Where(b => yetkiliPropertyIds.Contains(b.PropertyId));
